...
[__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$matcher$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$comparison$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$polymarket$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$kalshi$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$semantic$2d$matcher$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__] = __turbopack_async_dependencies__.then ? (await __turbopack_async_dependencies__)() : __turbopack_async_dependencies__;
;
;
;
;
...
async function GET(req) {
    ...
    const scanPromise = (async ()=>{
        const polyMarkets = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].market.findMany({
            where: {
                platform: 'POLYMARKET',
                active: true,
                volume24h: {
        ...
        await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$services$2f$semantic$2d$matcher$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SemanticMatcherService"].clearErrorCache(); // Limpiar evaluaciones erróneas del cache
        // PASO 1: Matches en paralelo (batches de CONCURRENCY)
        ...
        for(let i = 0; i < polyMarkets.length; i += CONCURRENCY){
            ...
            const batchResults = await Promise.all(batch.map(async (market)=>{
                try {
                    const entityKeywords = market.question.split(/\s+/).filter((w, idx)=>{
                    ...
                    }).map((w)=>w.toLowerCase().replace(/[^a-z]/g, '')).slice(0, 3);
                    if (entityKeywords.length === 0) return null;
                    const candidates = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].market.findMany({
                        where: {
                            platform: 'KALSHI',
                            active: true,
                    ...
                    const fullMarket = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].market.findUnique({
                        where: {
                            id: market.id
                        }
                    });
                    if (!fullMarket) return null;
                    const matches = matcherService.findMatchesFromCandidates(fullMarket, candidates, 0.6);
                    const topMatch = matches[0];
                    if (topMatch && topMatch.market.platform === 'KALSHI') {
                        // Validar semánticamente el par
                        const fullKalshiMarket = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].market.findUnique({
                            where: {
                                id: topMatch.market.id
                            }
                        });
                        if (!fullKalshiMarket) return null;
                        const semanticResult = await semanticMatcher.evaluatePair(fullMarket, fullKalshiMarket);
                        if (!semanticResult.isEquivalent) {
                            console.log(`[Scanner] 🤖 Semantic reject: "${fullMarket.question.slice(0, 40)}..." - ${semanticResult.reasoning}`);
                            return null;
                        }
                        console.log(`[Scanner] 🤖 Semantic confirm (${(semanticResult.confidence * 100).toFixed(0)}%): "${fullMarket.question.slice(0, 40)}..."`);
                        return {
                            polymarket: market,
                            kalshiMarket: {
                                id: topMatch.market.id,
                                externalId: topMatch.market.externalId,
                                slug: topMatch.market.slug,
                                volume24h: topMatch.market.volume24h ?? 0,
                                url: topMatch.market.url
                            },
                            matchScore: topMatch.score,
                            matchType: topMatch.matchType ?? 'RELATED'
                        };
                    }
                    return null;
                } catch (err) {
            ...
            for (const r of batchResults){
                if (r) {
                    confirmedPairs.push(r);
                    console.log(`[Scanner] ✅ Match: "${r.polymarket.question}" ↔ "${r.kalshiMarket.id}" (${r.matchScore.toFixed(2)})`);
                }
            }
        }
        console.log(`[Scanner] Scanned ${polyMarkets.length} Polymarket markets`);
        console.log(`[Scanner] Found ${confirmedPairs.length} matched pairs`);
        // PASO 2: Solo fetchear precios para los pares confirmados
        const opportunities = [];
        const minRoiFraction = parseFloat(minRoiParam);
        if (kalshiService) {
            const results = await Promise.allSettled(confirmedPairs.map(async (pair)=>{
                try {
                    ...
                    if (arbitrage.detected && arbitrage.roi !== null && arbitrage.roi / 100 >= minRoiFraction) {
                        opportunities.push({
                            id: `${pair.polymarket.id}-${pair.kalshiMarket.id}`,
                            question: pair.polymarket.question,
                            category: VALID_CATEGORIES.has(pair.polymarket.category ?? '') ? pair.polymarket.category : null,
                            roi: arbitrage.roi,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__450c09b5._.js.map
...
function resolveCategory(category, question) {
...
async function GET(req) {
    ...
    const scanPromise = (async ()=>{
        ...
        const confirmedPairs = verifiedPairs.map((pair)=>{
        ...
        const filteredPairs = confirmedPairs.filter((pair)=>{
        ...
        console.log(`[Scanner] ${confirmedPairs.length} verified pairs → ${filteredPairs.length} after volume filter`);
                ...
                await Promise.allSettled(batch.map(async (pair)=>{
                    try {
                        ...
                        if (arbitrage.detected && arbitrage.roi !== null && arbitrage.roi / 100 >= minRoiFraction) {
                            ...
                            const kalshiDbMarket = verifiedPairs.find((p)=>p.marketA.platform === 'KALSHI' && p.marketA.id === pair.kalshiMarket.id || p.marketB.platform === 'KALSHI' && p.marketB.id === pair.kalshiMarket.id);
                            const kalshiCategory = kalshiDbMarket?.marketA.platform === 'KALSHI' ? kalshiDbMarket.marketA.category : kalshiDbMarket?.marketB.category ?? null;
                            const resolvedCategory = VALID_KALSHI_CATEGORIES.has(kalshiCategory ?? '') ? kalshiCategory : null;
                            opportunities.push({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__34c6ba93._.js.map
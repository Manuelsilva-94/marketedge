...
/**
 * Builds the same JSON shape as GET /api/whales/[address].
 * Used by the API route and the whale detail server page to avoid self-fetch
 * (broken baseUrl on Vercel / localhost).
 */
export async function buildWhaleProfileResponse(
  normalized: string
): Promise<WhaleProfileApiResponse> {
  try {
